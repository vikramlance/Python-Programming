...
def will_infect(n,k,x,y):

    if y-x >= 0:
        result = 'YES' if (y-x) % k == 0 else 'NO'
    else:
        result = 'YES' if (y-x + n) % k == 0 else 'NO'
    return result
...
for test in range(t):
    ...
    n,k,x,y = sys.stdin.readline().strip().split(' ')
    n,k,x,y = [int(n),int(k),int(x),int(y)]
    
    print(will_infect(n,k,x,y) + '\n')
...
# print(will_infect(6,2, 5, 3))
# print(will_infect(12,3, 4, 2))